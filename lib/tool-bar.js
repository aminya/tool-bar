...
import { testIcons } from './icon-service/iconify';
...
export function activate() {
  ...
  // test iconify
  let mygroup = "TestIconify";
  let toolBarManager = new ToolBarManager(mygroup, toolBarView, touchBarManager);

  for (let testIcon of testIcons) {
    let mybutton = new ToolBarButtonView(testIcon, mygroup);
    toolBarManager.addItem({element: mybutton.element, priority: -1});
  }
